...
export default function Page() {
  ...
  function startMarkerPath(states: StateNode[], startState: string | null) {
    ...
    if (!s) return null
    const x1 = s.x - RADIUS - 45
    const x2 = s.x - RADIUS - 6
    const y = s.y
    // Draw line + arrowhead so the SVG path has non-zero geometric height
    return `M ${x1} ${y} L ${x2} ${y} M ${x2 - 8} ${y - 5} L ${x2} ${y} L ${x2 - 8} ${y + 5}`
  }
